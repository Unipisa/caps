...
import { BadRequestError, ValidationError, NotImplementedError, NotFoundError } from "../exceptions/ApiException"
import crypto from 'crypto'

const PERMISSION_SECRET = process.env.PERMISSION_SECRET || crypto.randomUUID()

function queryFieldsToPipeline(query:{[key:string]: string}={}, fields: {[key: string]: any}={}) {
    ...
    for (const key in query) {
        ...
        if (key == '_direction') {
            ...
            if (isNaN(limit) || limit < 0) throw new BadRequestError(`invalid _limit ${value}: positive integer expected`);
        } else if (key == '_sort') {
...
const ModelController = {
    ...
    index: async (Model: any, query:{[key:string]:string}, fields:{[key:string]:any}, {  populate = undefined } = {}) => {
    ...
    view: async (Model: any, id: string, { populate=undefined } = {}) => {
        try {
    ...
    patch: async (Model: any, id:string, data:any) => {
        try {
    ...
    idFromSignedIdIfValid: (signedId: string) => {
