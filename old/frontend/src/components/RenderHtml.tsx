...
function extractFieldNamesFromElement(el: HTMLElement) {
    const nodeName = el.nodeName.toLowerCase()
    if (nodeName === '#text') return []
    let names:string[] = []

    if (nodeName === 'input') names.push((el as HTMLInputElement).name)
    if (nodeName === 'select') names.push((el as HTMLSelectElement).name)
    if (nodeName === 'textarea') names.push((el as HTMLTextAreaElement).name)
    
    ;([...el.childNodes] as HTMLElement[]).forEach(child => {
...
function Error({key, children}: {key?: string, children: any}) {
...
function RenderElement({el, vars, data, setData}) {
    ...
    if (nodeName === 'tt') return <span style={{fontFamily: "monospace, monospace"}}>{children}</span>
